...
description: Gleiche die kanonische Projektplanung, Entscheidungen und dokumentierten Projektzustaende ab. Verwende diesen Skill bei `input aufnehmen`, `aktualisieren`, `tagesstart`, `Guten Morgen, es ist ein neuer Tag.`, `projektlage`, `chat-stats`, `chat-handover`, `update planung`, `plan aufnehmen`, `projektinput aufnehmen`, `entscheidung festhalten`, `council analyse`, `council review`, `council umsetzen`, `ohne council`, `nur Tera`, `mit Sol-Review` und bei Fragen nach der fuehrenden Projektwahrheit oder nach Dokumentduplikaten.
---
...
4. Pruefe neue Plaene oder Projektinputs nach `docs/project/PROJECT_INPUT_WORKFLOW.md`. Begrenze allgemeine Scans auf `git ls-files`.
