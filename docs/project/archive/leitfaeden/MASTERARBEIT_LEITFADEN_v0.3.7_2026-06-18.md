...
## Rolle der Software in der Masterarbeit
...
| `ma_economy` | geplant | Wirtschaftlichkeitsbewertung als eigenes Fachmodul |
...
- **Status:** Geplant; wesentliche Parameterlogik liegt derzeit in
  `ma_variants.parameter_catalog` und `ma_variants.option_catalog`.
...
- **Status:** Geplant; eine erste Uebergabestruktur existiert noch in
  `ma_variants.ida_export`. P006 beschreibt den weiteren Entwurf.
...
- **Abgrenzung:** Keine fachliche Kennwertberechnung, Diagrammerzeugung oder
  Bewertung.
- **Status:** Geplant; Teilfunktionen liegen derzeit in
  `ma_variants.simulation_results` und in der Import-/Aufbereitungslogik von
  `ma_analyse`.
...
- **Status:** Geplant; generische Wirtschaftlichkeitslogik liegt derzeit in
  `ma_variants.economic_analysis`.
