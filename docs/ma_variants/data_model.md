...
```text
config/ma_variants/products/example_products.yaml
config/ma_variants/materials/example_materials.yaml
config/ma_variants/documents/example_documents.yaml
config/ma_variants/sources/example_sources.yaml
```
...
```text
data/catalogs/documents/products/
data/catalogs/documents/materials/
```
...
- `data/ma_variants/exports/example_variants.json`
...
Die Beispielsysteme liegen unter `config/ma_variants/systems/example_system_templates.yaml`:
...
Eine Beispielvorlage ohne echte Zugangsdaten liegt unter `config/ma_variants/database/example.env`.
...
Die Produkt- und Materialdaten in den Beispielkatalogen sind Platzhalter. Sie dienen nur dazu, die Struktur fuer ausgewaehlte Varianten vorzubereiten. Datenblaetter werden nicht in PostgreSQL gespeichert, sondern ueber Pfade in `data/catalogs/documents` referenziert.
