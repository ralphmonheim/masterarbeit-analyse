...
Das bestehende Paket `ma_analyse` wertet bereits IDA-ICE-Simulationsergebnisse aus. Die aktuelle Pipeline liest Rohdaten aus `data/input`, erzeugt Nutzdaten unter `data/database` und schreibt Auswertungen nach `data/output` oder `data/test_output`.
...
Der Import liefert Python-Objekte fuer `Parameter`, `OptionSet` und `OptionValue`. Ein JSON-Importbericht wird standardmaessig nach `data/imports/import_report.json` geschrieben. In Tests kann ein eigener Berichtspfad uebergeben werden.
...
Die Varianten bleiben zunaechst reine Python-Objekte. Ein JSON-Export ist ueber `ma_variants.variant_manager.export_variants_to_json` moeglich. Eine kleine Beispielausgabe liegt unter `data/exports/example_variants.json`.
...
Die Namensgebung liegt unter `ma_variants.naming`. Namensregeln werden aus `config/naming/example_naming_rules.yaml` geladen. Die Beispielregeln nutzen:
...
Ausgewaehlte Varianten koennen mit `apply_variant_names` benannt werden. Dabei werden die erzeugten Namen auf Duplikate geprueft. Eine kleine Beispielausgabe fuer ausgewaehlte und benannte Varianten liegt unter `data/exports/example_selected_named_variants.json`.
...
`export_variant_overview` schreibt standardmaessig drei Dateien nach `data/exports`:
...
Abschnitt 7 fuehrt `ma_variants.system_catalog` ein. Der Systemkatalog liegt beispielhaft unter `config/systems/example_system_templates.yaml` und enthaelt kleine Vorlagen fuer:
...
Die Beispielkonfiguration liegt unter `config/export/example_ida_export.yaml`. Sie definiert:

- Zielordner `data/ida_exports`
...
Die Rohdatenaufbereitung `ma_analyse prepare` erwartet unter `data/input/<Variante>/<Raum>/` pro Raum diese IDA-ICE-PRN-Dateien:
...
```text
data/input/<Variante>/<Raum>/<PRN-Datei>
```

Aufbereitete Nutzdaten:

```text
data/database/<Variante>_nutzdaten/<raum_name>.csv
data/database/<Variante>_nutzdaten/<raum_name>.xlsx
```
...
```text
data/output/<Variante>/<run_id>_output/
```
...
Der neue Adapter liest zunaechst nur die aufbereiteten Raum-CSV-Dateien aus `data/database/<Variante>_nutzdaten`. Bestehende PNG-, PDF- und XLSX-Ausgaben werden nicht als Quelle benoetigt.
...
Die Ergebnisse koennen als JSON und CSV nach `data/exports` geschrieben werden:

- `variant_cost_results.json`
- `variant_cost_results.csv`

Die Annahmen, Formeln und Grenzen sind zusaetzlich in `docs/ECONOMIC_MODEL.md` dokumentiert.
...
Die UI enthaelt keine eigene Fachlogik. Sie zeigt Parameter und Optionen an, berechnet die Variantenanzahl ueber den Variantenmanager, erzeugt die vorhandenen Beispielvarianten, erlaubt eine manuelle Variantenauswahl, startet den Basisexport und listet vorhandene Ergebnisdateien aus dem Exportordner.
...
4. Eine oder mehrere Varianten auswaehlen.
5. Export starten.
6. Im Tab `Ergebnisse` pruefen, ob `selected_variants.json`, `selected_variants.csv` und `variant_report.json` angezeigt werden.

## Geplante Datenbereiche

- `config/`: bearbeitbare Projektkonfigurationen fuer Parameter, Optionen, Systeme, Namensregeln, Wirtschaftlichkeit, Produkte, Materialien, Dokumente und Quellen.
- `data/input/`: vorhandene IDA-ICE-Rohdaten und Variantenordner.
- `data/database/`: vorhandene aufbereitete Nutzdaten aus der Analysepipeline.
- `data/output/`: vorhandene regulaere Analyseausgaben.
- `data/test_output/`: vorhandene Test- und Experimentausgaben.
- `data/imports/`: Zielordner fuer Importberichte.
- `data/exports/`: Zielordner fuer Variantenuebersichten, Basisexporte, Wirtschaftlichkeitsergebnisse und Exportberichte.
- `data/ida_exports/`: Zielordner fuer vorbereitete IDA-ICE-Uebergabestrukturen je Variante.
- `data/documents/`: Zielordner fuer referenzierte Produkt- und Materialdatenblaetter; Dateien werden nicht in PostgreSQL gespeichert.
- `data/reports/`: spaeterer Zielordner fuer Berichte.
