...
from ...core.config import DATENBANK_DIR, OUTPUT_DIR, ROOM_FILE_EXTENSION, ROOMS, RUN_FOLDER_SUFFIX
...
def load_room_csv(csv_file, debug=False):
    ...
    try:
        ...
        # Neue Header aus prepare.py auf die erwarteten Plotspalten abbilden.
...
def build_run_output_dir(variant_dir, run_id, output_root=None):
    ...
    # Falls kein eigenes Root übergeben wird, schreiben wir standardmäßig in data/output/.
    # Die Variante liegt jetzt direkt unter data/output/, der konkrete Lauf darunter.
    # Dadurch wird die Struktur kuerzer: data/output/<variante>/<run_id>/...
